import express from 'express';
  ...
  import { freeStorage } from '@grammyjs/storage-mongodb';
  ...
  bot.use(async (ctx, next) => {
    ...
    ctx.sessionStorage = freeStorage<SessionData>(db?.collection('sessions'));
    await next();
  });
  ...
  bot.command('start', async (ctx) => {
    await ctx.reply(
      ...
      {
        reply_markup: new InlineKeyboard().webApp('Voice Recognition', process.env.MINI_APP_URL!)
      }
    );
  });
